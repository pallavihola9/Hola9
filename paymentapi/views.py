...
def callback(request):
	...
	if status=="Transaction Successful" and s.paymentStatus != "Success":
		...
		planJsonOBJ1 = open('/hola9Main/hola9DjangoLatest/paymentapi/json/businessPlan.json','r+', encoding="utf-8")
		...
		if s.plan=="Premium":
			...
			x=BusinessPricing.objects.create(user=user,category=category,validity=validity,visiblity=visiblity,NoAds=s.adsValue,OrderID=OrderID,city=s.City)
			...
			x=BusinessPricing.objects.create(user=user,category=category,validity=validity,city=city,visiblity=visiblity,OrderID=OrderID)
			finalData["category"]=category
			finalData["validity"]=validity
			finalData["city"]=city
			finalData["visiblity"]=visiblity
			...
			x=Pricing.objects.create(user=user,category=category,days=days,regulars=regulars,topAds=topAds,featured=featured,teleSupport=teleSupport,response=response,chatSupport=chatSupport,dedicatedRm=dedicatedRm,hol9Website=hol9Website,OrderID=OrderID,validity=validity)
			finalData["days"]=days
...
class create_payment_token_Class(APIView):
	def post( self,request , format=None):
		...
		planJsonOBJ = open('/hola9Main/hola9DjangoLatest/paymentapi/json/businessPlan.json','r+', encoding="utf-8")
		...
		if error == "":
			...
			layer_params = "{payment_token_id:"+payment_token_data["id"]+",accesskey:"+accesskey+"}"
			token_id=payment_token_data["id"]
		s=TransactionDetails.objects.create(payment_token_id=token_id,payment_id="",userID_id=name,plan=planValue,paymentStatus="Cancelled",email=data["email_id"],tranid=data["mtx"],order_payment_amount=data["amount"],phoneNumber=data["contact_number"],adsValue=adsValue,monthsVale=monthsVale,City=request.data.get("city"))
